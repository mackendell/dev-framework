...
- **Framework-native skills** (`dev`, `continue`, `checkpoint`, `design-gate`, `verify-acs`, `questionnaire`, `model-routing`, `check-upstream`) are ours; keep them consistent with the routes named in `skills/dev/SKILL.md`.
...
- **Hooks** (`hooks/`) are the deterministic layer over the skills: the Stop guard assumes `/checkpoint` exists and `.dev/PROGRESS.md` is the progress index — renaming either means updating the hook scripts.
